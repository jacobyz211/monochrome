...
function buildConfigPage(baseUrl) {
  ...
  h += '<footer>Claudochrome Eclipse Addon v1.4.0 &bull; Hi-Fi API v2.7</footer>';
  h += '<script>';
  h += 'var _gu="",_ru="";';
...
app.get('/health', function(_req, res) {
  res.json({ status: 'ok', version: '1.4.0', hifiApiVersion: '2.7', activeInstance: activeInstance, instanceHealthy: instanceHealthy, activeTokens: TOKEN_CACHE.size, redisConnected: !!(redis && redis.status === 'ready'), timestamp: new Date().toISOString() });
});
...
// ─────────────────────────────────────────────────────────────────────────────
// SEARCH
// Accepts ?q=, ?query=, or ?s= (all three) so it works regardless of what Eclipse sends
// v2.7 /search/ always returns tracks — albums/artists extracted from track fields
// ─────────────────────────────────────────────────────────────────────────────
app.get('/u/:token/search', tokenMiddleware, async function(req, res) {
  // Accept any of the three common search param names Eclipse might use
...
// ─────────────────────────────────────────────────────────────────────────────
// STREAM
// v2.7: GET /track/?id=<id>&quality=LOSSLESS → base64 manifest → decode → URL
// ─────────────────────────────────────────────────────────────────────────────
...
// Shape: data.data = album, data.data.items[].item = track
// ─────────────────────────────────────────────────────────────────────────────
app.get('/u/:token/album/:id', tokenMiddleware, async function(req, res) {
  ...
  try {
    ...
    for (var i = 0; i < rawItems.length; i++) {
      var t = rawItems[i].item || rawItems[i];   // unwrap nested .item
...
// ─────────────────────────────────────────────────────────────────────────────
// ARTIST
// v2.7 needs TWO calls:
//   GET /artist/?id=<id>                        → data.artist (name/pic), data.cover["750"]
//   GET /artist/?f=<id>&skip_tracks=false       → data.albums.items, data.tracks[]
// NOTE: f= param must be an integer artist ID
// ─────────────────────────────────────────────────────────────────────────────
app.get('/u/:token/artist/:id', tokenMiddleware, async function(req, res) {
  ...
  try {
    ...
    for (var j = 0; j < sortedAlbums.length; j++) {
      ...
      albums.push({ id: String(al.id), title: al.title || 'Unknown', artworkURL: coverUrl(al.cover), trackCount: al.numberOfTracks || null, year: (al.releaseDate || '').slice(0, 4) || null });
    }
...
// Shape: data.playlist = metadata, data.items[].item = track
// ─────────────────────────────────────────────────────────────────────────────
app.get('/u/:token/playlist/:id', tokenMiddleware, async function(req, res) {
  ...
  try {
    ...
    res.json({
      ...
      artworkURL: pl.squareImage ? coverUrl(pl.squareImage, 480) : null,
...
app.listen(PORT, function() {
  console.log('Claudochrome v1.4.0 (Hi-Fi API v2.7) on port ' + PORT);
});
